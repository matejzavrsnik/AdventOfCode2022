...
assignment_pairs convert(std::string s);
bool one_inside_other(std::pair<int, int> one, std::pair<int, int> other);
bool is_fully_contained (assignment_pairs assignments);
bool one_overlaps_other(std::pair<int, int> one, std::pair<int, int> other);
bool is_overlapping (assignment_pairs assignments);

// part 1 solution
long fully_contained(std::string input_file)
{
   auto input = mzlib::read_file_lines(input_file);

   std::vector<assignment_pairs> assignments;
   std::transform(input.begin(), input.end(), std::back_inserter(assignments),
      [](auto s){ return convert(s); });

   int count = std::count_if(assignments.begin(), assignments.end(),
      [](auto a){ return is_fully_contained(a); });
...
long overlapping(std::string input_file)
{
   ...
   int count = std::count_if(assignments.begin(), assignments.end(),
      [](auto a){ return is_overlapping(a); });
...
assignment_pairs convert(std::string s)
{
   ...
   return {
      {std::stoi(assignment1[0].data()), std::stoi(assignment1[1].data())},
...
{
   return
      one_inside_other(assignments.first, assignments.second)
...
{
   return
      one_overlaps_other(assignments.first, assignments.second)
